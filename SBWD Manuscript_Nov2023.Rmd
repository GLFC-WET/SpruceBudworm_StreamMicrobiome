...
waterchem_long1 <- waterchem_long %>%
  replace_with_na(replace = list(value = 3.420800))

# need to edit here to pull in from raw github data not intermediate

flow_fullseason_means <- readRDS("~/SBWD_data_analysis/EditedData/StreamHabitat/flow_fullseason_means.RDS")

# need to edit here to pull in from raw github data not intermediate

temp_levellogger_summary <- readRDS("~/SBWD_data_analysis/EditedData/StreamHabitat/temp_levellogger_summary.RDS")
...
#I wll add the appropriate file to this repository upon acceptance, but using a local copy for now
...
metadata_lp_its <- metadata_lp_its %>%
 inner_join(Mortality_cats, by = c("site", "year"))
...
#saveRDS(metadata_lp_its, file = "metadata_lp_its.RDS")
#reorder sites
...
## Algal Biomass Models
 ...
 - incubation period only to align with fungi and bacteria 

```{r}
#create waterchem incmeans wide from streams


algae <- waterchem_incmeans_wide %>%
  filter(year != "2020") %>%
  dplyr::select(1,2,3,9,10,13,22)
...
* XY, NAG, PHOS, and CELO enzyme activity plotted per site, per year
* CELO added for manuscript (cellubiodase)
* 1 leafpack missing in 2021 (no leaves in leafpack)

## Weight corrected enzyme activities

```{r}
#leafpacks <- readRDS("~/SBW Data/SBWD_master-main_2023May15/SBWD_master-main/data/leafpacks.RDS")

#NAG, PHOS, and XY
#enzymes_2019_2021 <- dplyr::select(leafpacks, -inc_days, -mass.loss.g, -percent.loss, -cpom.g, -fpom.g, -cpim.g, -fpim.g)

#long format
#enzymes <- enzymes_2019_2021 %>% pivot_longer(cols = c(4,5,6), names_to = "enzyme", values_to = "activity.nmol.h.g")

#enzymes$enzyme <- gsub("\\..*", "", enzymes$enzyme) 

#enzymes_3 <- enzymes %>%
#  dplyr::select(1,3, 2, 4, 5)

#pull in enzymes file - add to repo and pull from there
