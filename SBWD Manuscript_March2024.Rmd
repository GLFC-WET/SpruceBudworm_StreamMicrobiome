...
- defoliation, stream habitat, enzyme, watershed characteristic, and any other supporting data is located in the data folder of this repository 
- 16S and ITS sequence data is on NCBI (PRJNA1041793) and was processed using the MetaWorks Pipeline and a standard QAQC process(see rmd files: QAQC_SBWD_16S_2022_07_12 and QAQC_SBWD_ITS_2022_10_26). See paper methods for more details 
...
watersheds_hydroweight_statistics <- readRDS("C:/Users/mmccaig/OneDrive - NRCan RNCan/Documents/GitHub/SpruceBudworm_StreamMicrobiome/data/watersheds_hydroweight_statistics.RDS")
...
streams <- readRDS("~/GitHub/SpruceBudworm_StreamMicrobiome/data/streams.RDS")
...
- microbiome sequence data is available on NCBI and was processed with the MetaWorks Pipeline (see methods for more details) 
- the outputs from MetaWorks are processed through the QAQC scripts (see the rmd files - QAQC_SBWD_16S_2022_07_12 and QAQC_SBWD_ITS_2022_10_26) and the final products (lp_16s and lp_its) are pulled in here 


```{r}
### Bacteria (16S)
lp_16s <- readRDS("~/GitHub/SpruceBudworm_StreamMicrobiome/data/lp_16s.RDS")
```
...
#full season flow 
field <- readRDS("~/GitHub/SpruceBudworm_StreamMicrobiome/data/field.RDS")
...
```{r load its}
### Phyloseqs - Fungi (ITS)
lp_its <- readRDS("~/GitHub/SpruceBudworm_StreamMicrobiome/data/lp_its.RDS")
```
...
```{r}
enzymes <- readRDS("~/GitHub/SpruceBudworm_StreamMicrobiome/data/enzymes_Oct2022.RDS")
...
SBWD_2019_21_CELO <- read_csv("~/GitHub/SpruceBudworm_StreamMicrobiome/data/SBWD_2019-21_CELO.csv", col_types = cols(...10 = col_skip()))
...
SBWD_2019_21_Moisture <- read_csv("~/GitHub/SpruceBudworm_StreamMicrobiome/data/SBWD_2019-21_Moisture.csv")
...
```{r Picrust Data Prep, eval = FALSE }
####### Data Export for piCrust analysis #######
####16S data to be reformatted

#when running on linux, just use the picrust script to run the full pipeline, but use -p 20 cores rather than 1
# output 
# - ASV as first column, Samples as subsequent columns
# - tab delimited
# - no row names
...
metadata_lp_16s <- cbind(rownames(metadata_lp_16s), data.frame(metadata_lp_16s, row.names=NULL))

metadata_lp_16s <- metadata_lp_16s %>%
  rename(sampleid = 'rownames(metadata_lp_16s)')

site_search <- str_c(metadata_lp_16s$sampleid, collapse = "|")
outtab <- lp_16s[ , c("ESV", colnames(lp_16s)[grepl(site_search, colnames(lp_16s))])]

#this file is an output of the MetaWorks pipeline and can be reproduced using the sequences on NCBI
fas_out <- readLines("~/SBWD/data/16S_cat.denoised.nonchimeras")

#what line number does each zotu start on
Zotus_fas <- data.frame(Zotu = str_extract(grep(">", fas_out, value=T), 
                                           "Zotu[[:digit:]]+"), 
                        linenum = grep(">", fas_out))
#what line number does it end on
Zotus_fas$lastln <- c(Zotus_fas$linenum[2:nrow(Zotus_fas)]-1, nrow(Zotus_fas))

#match fasta to the outtab file (remove extra zotus)
Zotus_fas <- Zotus_fas[Zotus_fas$Zotu %in% outtab$ESV,]

outtab<- outtab[match(Zotus_fas$Zotu, outtab$ESV), ]
#what is the range of line numbers
Zotus_fas$linerng <- sapply(1:nrow(Zotus_fas), function(x){Zotus_fas$linenum[x]:Zotus_fas$lastln[x]}) 

#turn that line range into a vector not a list
linesin <- unlist(Zotus_fas$linerng)

#link the OG fasta file to the line range
out_fas16s <- fas_out[linesin]

write(out_fas16s, "data/lp_16S_forPicrust.fasta")
write.table(outtab, "data/lp_16S_tableforPicrust.txt", sep="\t", row.names = F, eol="\r", fileEncoding="UTF-8")

rm(lp_16s, out_fas16s, outtab, fas_out, linesin, Zotus_fas)

#run 16s in linux
#picrust2_pipeline.py -s study_seqs.fna -i study_seqs.biom -o picrust2_out_pipeline -p 20
...
picr16s <- read.table(file = '~/GitHub/SpruceBudworm_StreamMicrobiome/data/path_abun_unstrat.tsv', sep = '\t', header = TRUE)

#metacyc ontology
metacyc <- read.delim(file = '~\\GitHub\\SpruceBudworm_StreamMicrobiome\\data\\MetaCyc_Ontology.txt')
...
#remove superpathways (they are now combined in with their respective groups, don't need separate category for general super pathways)
